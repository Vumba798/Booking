...
object DatabaseActor {
  ...
  private def bookingCommandsHandler(c: BookingCommands): Behavior[Command] =
    c match {
  ...
  private def authCommandsHandler(c: AuthCommands): Behavior[Command] =
    c match {
      // TODO
      case _ => Behaviors.same
    }

  private def analyticsCommandsHandler(
      c: AnalyticsCommands
